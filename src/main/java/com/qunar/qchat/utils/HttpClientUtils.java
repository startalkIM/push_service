...
/**
    ...
    public static String postBody(String url, String body) {
        checkArgument((StringUtils.isNotEmpty(url) && StringUtils.isNotEmpty(body)), "url or jsonBody should not be null or empty");
        HttpClient hc = initHttpClient();
        HttpPost post = new HttpPost(url);
        ...
        try {
            ...
            if (status == HttpStatus.SC_OK) {
                ...
                LOGGER.info("post json is not ok,status:{},url:{},jsonBody:{}", status, url, body);
            }
        } catch (IOException e) {
            LOGGER.info("post json error,url:{},jsonBody:{}", url, body, e);
        } finally {
            post.releaseConnection();
        }
        return res;
